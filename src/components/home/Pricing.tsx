      ...
      <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-5xl md:text-6xl lg:text-7xl font-extrabold text-white tracking-tighter leading-none font-display mb-4">
            The right <span className="text-yellow-400">Plans</span> for <span className="relative">your <span className="text-yellow-400">Ideas</span>
              <svg className="absolute -top-6 -right-8 w-12 h-12 text-yellow-400" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {pricingPlans.map((plan, index) => (
            <div
              key={index}
              className={`relative rounded-lg overflow-hidden ${plan.popular ? 'bg-gradient-to-br from-yellow-500 to-yellow-600' : 'bg-dark-800 border border-dark-700'
                }`}
            >
              {plan.popular && (
                <div className="absolute top-0 right-0">
                  <div className="bg-yellow-600 text-black font-bold py-1 px-4 text-sm">
                    MOST POPULAR
                  </div>
                </div>
              )}
              ...
              <div className="p-8">
                ...
                <div className="inline-block bg-black text-yellow-400 text-xs font-bold px-2 py-1 rounded mb-4">
                  {plan.discount}
                </div>
                  ...
                  <ul className="space-y-3">
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-start">
                        <CheckCircleIcon className={`h-5 w-5 mr-2 flex-shrink-0 ${plan.popular ? 'text-black' : 'text-yellow-400'
                          }`} />
